...
// Decode budget: where a generated token's time goes, from the engine's per-token
// timeline (DS4_ARGODRIVE_TIMELINE) when the run recorded one. Six segments that sum
// to the token; "other" is CPU/GPU bubbles plus the final drain.
const budgetSegments=[['gpu','GPU','var(--accent)'],['misses','Expert misses','var(--orange)'],['head','Head','var(--blue)'],['engram','Engram','var(--purple)'],['prepare','Prepare','var(--teal)'],['other','Other','var(--faint)']];
function decodeBudget(b,r){
  if(!b||!finite(b.total)||b.total<=0)return '';
  const parts=budgetSegments.map(([k,label,color])=>{const ms=finite(b[k])?Math.max(0,b[k]):0;return {label,color,ms,pct:100*ms/b.total};});
  const bar=parts.filter(p=>p.pct>0).map(p=>`<i style="width:${p.pct.toFixed(2)}%;background:${p.color}" title="${esc(p.label)} · ${val(p.ms,1)} ms · ${val(p.pct,0)}%"></i>`).join('');
  const legend=parts.map(p=>`<span><i class="legend-dot" style="background:${p.color}"></i>${esc(p.label)} <strong>${val(p.ms,1)} ms</strong> · ${val(p.pct,0)}%</span>`).join('');
  const steady=finite(b.steady_tok_s)?b.steady_tok_s:r?.tok_s_steady;
  return `<h3>Decode budget</h3><div class="budget-bar" role="img" aria-label="Where each generated token's time goes, as a share of the token">${bar}</div><div class="chart-legend budget-legend">${legend}</div><p class="budget-line">${val(steady)} tok/s steady · ${val(b.total,1)} ms/token · ${val(b.cbs_per_token,1)} command buffers/token · ${val(b.misses_per_token,1)} misses/token</p><p class="chart-note">Mean over ${val(b.tokens,0)} generated tokens, after the prefill and the first two tokens. Recorded by the engine; not a harness estimate.</p>`;
}
...
function monitorHeading(){
  ...
  return heading('Monitor','Choose current hardware activity or a test’s recorded measurements.',
    ...
    `<div class="monitor-source-note" title="${hardware?'Includes other apps and background work.':'Switching sources does not start a test or change hardware collection.'}"><span>${hardware?'Live · system-wide reads':'Runner-recorded SSD reads'} · GB/s · axis fits the window: 10% above average, never below peak · solid average / dashed peak${state.monitorShowEngram?' · <i class="monitor-key weights"></i> weights / <i class="monitor-key engram"></i> Engram':''}</span><label class="monitor-combined-toggle"><input type="checkbox" id="monitor-combined" ${state.monitorCombined!==false?'checked':''}> Show combined</label></div>`;
}
...
function updateHarness(d){
  ...
  $('#harness-content').innerHTML=`<div class="summary-strip"><span>${badge(d.state,d.fresh?'green':d.done?'purple':'amber')} <strong style="margin-left:10px">${d.model_path?esc(d.model_path.split('/').at(-1).replace(/\.gguf$/,''))+' · ':''}${esc(d.arm)}</strong></span><span>${d.kind==='task'?'Task activity · no token-speed measurement':`${d.requested_tokens||'—'} requested tokens · ${d.kind==='ds4-bench'?(d.generated_tokens??'—')+' generated tokens':(d.chunks||0)+' response chunks'}`}</span></div><p class="chart-note monitor-run-note">${esc(d.block)} · ${d.done?'Recorded window retained; this task or arm is finished.':d.catching_up?'Reading the existing history…':'File activity '+age+' ago. Engine phase is not inferred from SSD activity.'}</p>`+
    ...
    (drives.length&&(state.monitorShowSSD||state.monitorShowEngram)?`${state.monitorShowEngram&&!hasAttribution(d,drives)?`<div class="monitor-attribution-note"><strong>Weights + Engram overlay enabled</strong><span>${state.monitorShowSSD?'Waiting for per-request class telemetry. Grey bars remain device-wide until the engine supplies class intervals.':'Waiting for per-request class telemetry; SSD bars are hidden until selected.'}</span></div>`:''}<div class="live-drive-stack">${diskCards}${state.monitorShowSSD&&state.monitorCombined!==false?combinedReadCard(d.read_windows?.TOTAL,barScale,state.liveWindow,drives.length,d.fresh&&state.monitorSource==='latest'?tot.points.at(-1)?.[1]:null,drives.reduce((s,x)=>s+(finite(x.stats?.mean)?x.stats.mean:0),0)):''}</div>`:empty(state.monitorShowSSD?'No host SSD counters':'No chart layer selected',state.monitorShowSSD?(d.timeline_note||'The arm has not recorded physical-drive samples yet.'):'Enable SSDs or Monitor Engram above.'))+
...
function combinedReadCard(points,scale,seconds,count,current=null,capTotal=null){
  ...
  // Axis = the sum of the per-drive AVERAGES shown on the drive cards (KP, 2026-09-15),
  // so the combined line sits against the total the disks are actually averaging. Peaks
  // above that widen the axis rather than clip. Floor of 4 so an idle machine still
  // draws a chart; falls back to the fixed per-drive axis x count only if no drive
  // reported a window at all.
  // 10% above the summed average, never below the combined peak, floor 4 when idle.
  const avgTotal=finite(capTotal)&&capTotal>0?capTotal*1.10:0;
  const totalScale={end:scale.end,ceiling:Math.max(4,Math.ceil(Math.max(avgTotal,peak||0)))||scale.ceiling*Math.max(1,count)};
